...
exports.run = async (client, message, args) => {
    ...
    if(!rank){
        ...
        newrank = await getRankFromName(Number(process.env.groupId));
    } else {
        newrank = rank;
    }
   let reason = args.slice(2).join(" ");
    ...
    message.channel.send(`**Success!** Ranked ${username} from ${rankNameInGroup} (${rankInGroup}) to ${setRankResponse.name} (${setRankResponse.rank})`);
    ...
    logchannel.send({embed: {
        color: 2127726,
        description: `${username} has been ranked to ${setRankResponse.name}.
